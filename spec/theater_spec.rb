describe Theater do
  ...
  %w[morning day evening].each do |time|
    it '#show {morning/day/evening}' do
      theater.customer = customer
      customer.deposit(money)
      possible_movies = theater.movies_collection.filter(theater.filter(time)).map do |movie|
